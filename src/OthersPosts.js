...
function OthersPosts({ currentUser }) {
    ...
    function showOtherPosts(data) {
        ...
        const theOtherPosts = otherUsers.map(user => <Post key={user.id} act={user.act} title={user.title} user={user.user} like={user.like} IDidIt={user.IDidIt} IWillDoIt={user.IWillDoIt} />)
